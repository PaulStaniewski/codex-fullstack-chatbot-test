...
LESSONS.update(
    {
        "backend_api_design_basics": _beginner_lesson(
            ...
            difficulty="easy",
            intro="A backend API exposes predictable operations for clients while hiding internal implementation details.",
            concept="Good API design uses clear resources, HTTP methods, status codes, and response shapes.",
            example="GET /conversations lists conversations. POST /conversations creates one. GET /messages?conversation_id=1 reads messages.",
            practice="Pick a resource in a chatbot app and describe one GET endpoint and one POST endpoint for it.",
            summary="You learned how resources, methods, and response shapes make APIs easier to use.",
        ),
        "backend_user_owned_resources": _beginner_lesson(
            ...
            difficulty="easy",
            intro="Many backend records belong to a specific user, including conversations, messages, lesson progress, and practice submissions.",
            concept="A user-owned resource should be created with the authenticated user's ID and read only through user-scoped queries.",
            ...
            summary="You learned the basic ownership rule behind safe multi-user backend APIs.",
        ),
        "database_transaction_basics": _advanced_lesson(
            ...
            difficulty="medium",
            intro="A transaction groups database changes so they succeed or fail together.",
            concept="Transactions protect consistency when a request writes multiple related rows.",
            deep_dive="Without rollback, an exception can leave partial state. Without careful commit timing, later failures may not undo earlier writes.",
            example="try:\n    db.add(order)\n    db.add(audit_log)\n    db.commit()\nexcept Exception:\n    db.rollback()\n    raise",
            checklist="Define transaction boundaries, commit once, rollback on errors, keep transactions short, and test partial failure paths.",
        ...
        "backend_data_isolation_audit": _advanced_lesson(
            ...
            difficulty="production",
            intro="A data isolation audit checks whether one user can ever read, modify, export, or infer another user's data.",
            concept="Isolation must be enforced in every endpoint, background task, export path, and history query.",
            deep_dive="Security bugs often hide in secondary features: export endpoints, progress views, rename/delete actions, and practice history pages.",
            example="Test with user A and user B. Create data as A, then attempt to read it as B through every endpoint.",
            checklist="Inventory user-owned tables, review every query, add two-user tests, check exports, and log suspicious access attempts.",
        ...
        "backend_migration_release_strategy": _advanced_lesson(
            ...
            difficulty="production",
            intro="Schema changes and application deploys must be ordered so old and new code can survive the transition.",
            concept="Safe migrations are backward compatible when possible and are deployed before code that requires the new schema.",
            deep_dive="Breaking deploys happen when code expects a column before migration, removes a column still used by old code, or runs irreversible data changes without rollback planning.",
            ...
            checklist="Plan expand-and-contract changes, run migrations in CI, back up data, monitor deploys, and document rollback options.",
        ...
        "frontend_component_state_basics": _beginner_lesson(
            ...
            difficulty="easy",
            intro="Interactive React screens need state for selected items, form inputs, loading, and errors.",
            concept="Component state should represent what the UI needs to render right now.",
            ...
            summary="You learned how local state drives basic React interactions.",
        ),
        "frontend_api_client_basics": _beginner_lesson(
            ...
            difficulty="easy",
            intro="A frontend API client centralizes fetch behavior so components do not repeat headers, parsing, and error handling.",
            concept="A small apiFetch helper can attach auth tokens, parse JSON, and throw friendly errors.",
            example="apiFetch('/conversations', { token })",
            practice="Describe why centralizing API calls is safer than calling fetch differently in every component.",
            summary="You learned how an API helper keeps frontend data access consistent.",
        ),
        "frontend_auth_token_storage": _advanced_lesson(
            ...
            difficulty="medium",
            intro="Frontend apps need to remember auth state across refreshes without exposing tokens carelessly.",
            concept="localStorage is simple, but tokens stored there can be read by injected scripts if XSS exists.",
            deep_dive="Token storage is a tradeoff among convenience, XSS risk, CSRF behavior, refresh UX, and backend auth design.",
            example="localStorage.setItem('chatbot_access_token', token)",
            checklist="Use HTTPS, keep tokens short-lived, avoid rendering unsafe HTML, clear tokens on logout, and handle expired sessions.",
        ...
        "frontend_markdown_rendering": _advanced_lesson(
            ...
            difficulty="medium",
            intro="AI answers often include headings, lists, links, and code blocks that are hard to read as plain text.",
            concept="Markdown rendering improves readability, but rendered content must be treated as untrusted input.",
            deep_dive="Syntax highlighting, link behavior, HTML escaping, and copy buttons all affect the safety and usability of AI-generated content.",
            example="<ReactMarkdown>{assistantMessage}</ReactMarkdown>",
            checklist="Escape raw HTML, style code blocks, handle long lines, open external links safely, and keep user messages plain text.",
        ...
        "frontend_stream_reconnect_strategy": _advanced_lesson(
            ...
            difficulty="production",
            intro="Streaming connections can fail because of network changes, expired tokens, provider issues, or browser behavior.",
            concept="A reconnect strategy decides when to retry, when to stop, and how to protect message ordering.",
            deep_dive="Blind reconnects can duplicate prompts, append repeated chunks, or keep retrying after auth failure. Safe reconnects need state and limits.",
            example="Retry only if no final assistant message was saved and the error is retryable.",
            checklist="Classify errors, cap retries, preserve drafts, avoid duplicate messages, close old EventSource objects, and show user control.",
        ...
        "frontend_security_xss_markdown": _advanced_lesson(
            ...
            difficulty="production",
            intro="Rendering AI or user-provided content can become dangerous if the app allows scripts or unsafe HTML.",
            concept="XSS happens when untrusted content executes as code in the browser.",
            deep_dive="Markdown libraries vary in how they handle raw HTML, links, and plugins. A safe app assumes AI output is untrusted.",
            example="Render markdown without raw HTML and use safe link attributes for external URLs.",
            checklist="Disable raw HTML, sanitize if needed, avoid dangerouslySetInnerHTML, protect tokens, review link behavior, and test malicious content.",
...
LESSONS.update(
    {
        "ai_prompting_basics": _beginner_lesson(
            ...
            "AI Prompting Basics",
            difficulty="easy",
            intro="A prompt tells the model what role to take, what context to use, and what kind of answer to produce.",
            concept="Good prompts are specific about task, audience, constraints, and output style.",
            ...
            summary="You learned how clearer prompts produce more useful AI responses.",
        ),
        "ai_context_window_basics": _beginner_lesson(
            ...
            difficulty="easy",
            intro="Models can only use the information included in their current context window.",
            concept="Context includes system instructions, user messages, retrieved content, and previous conversation history sent to the model.",
            example="For lesson tutoring, include the lesson title, step title, step content, and user question.",
            practice="List the minimum context an AI tutor needs to answer a question about a specific lesson step.",
            summary="You learned why AI features must choose context intentionally.",
        ),
        "openai_streaming_basics": _advanced_lesson(
            ...
            difficulty="medium",
            intro="Streaming sends model output in small chunks so users see progress before the full answer is complete.",
            concept="The backend receives chunks from the provider and forwards them to the browser over SSE or another streaming transport.",
            deep_dive="Streaming improves perceived speed, but it complicates errors, final persistence, retries, and UI state.",
            example="async for chunk in stream:\n    yield format_sse_data(chunk)",
            checklist="Stream chunks safely, track final state, handle provider errors, close client connections, and avoid saving partial output on failure.",
        ...
        "ai_prompt_builder_design": _advanced_lesson(
            ...
            difficulty="medium",
            intro="Prompt builders keep AI behavior consistent across routes and features.",
            concept="A prompt builder converts product state into system and user messages for the model.",
            deep_dive="Duplicated prompt strings drift over time. Shared builders make it easier to test lesson context, practice instructions, and safety constraints.",
            example="build_lesson_tutor_prompt(lesson, step, question)",
            checklist="Separate system behavior from user context, include source-of-truth content, avoid secrets, and test important prompt fields.",
        ...
        "invalid_json_from_model": _advanced_lesson(
            ...
            difficulty="hard",
            intro="Models sometimes return malformed JSON even when asked for structured data.",
            concept="AI output should be treated as untrusted text until parsed and validated.",
            deep_dive="Malformed JSON can come from extra prose, trailing commas, truncated streams, or fields with unexpected types.",
            example="{\"score\": 90, \"strengths\": [\"Clear\"],}",
            checklist="Parse defensively, validate schema, keep raw output, return safe errors, and test common malformed cases.",
        ...
        "ai_safe_error_handling": _advanced_lesson(
            ...
            difficulty="production",
            intro="AI provider failures should not expose secrets, stack traces, or confusing technical details to users.",
            concept="Safe errors separate user-facing recovery from operator-facing diagnostics.",
            deep_dive="Provider auth failures, rate limits, network timeouts, and client disconnects all need different logs but similarly safe UI messages.",
            example="data: Error: Unable to generate response.",
            checklist="Catch known provider errors, log clear internal causes, return safe messages, avoid persisting failed output, and offer retry when appropriate.",
        ...
        "ai_evaluation_and_regression_tests": _advanced_lesson(
            ...
            difficulty="production",
            intro="AI features change over time as prompts, models, and product requirements evolve.",
            concept="Evaluation checks whether AI outputs still meet product expectations for important scenarios.",
            deep_dive="Regression tests for prompt builders, parsers, safety behavior, and representative examples catch breakage without calling real providers in every test.",
            example="Assert the prompt includes lesson title, step content, and user answer before streaming feedback.",
            checklist="Test prompt inputs, mock provider streams, include failure cases, track quality examples, and review changes before model upgrades.",
